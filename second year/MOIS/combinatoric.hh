...
class combinatoric{
    ...
    vector<vector<int>> generate_permutations_from_N_to_M(int n, int m){
        ...
        cout<<"количесвто перестановок из "<<n<<" элементов в которых " <<m<<" элемента на своём месте: "<<count_perm<<endl;
    ...
    void print_traveling_salesman(int arr[], vector<vector<int>> matrix, int start) {
        ...
        // Функция генерации перестановок
    void traveling_salesman(int arr[], int n, vector<vector<int>> matrix,int start,int k = 0) {//перестановки для задачи комивояжёра
