...
class DatabaseApp:
    ...
    def create_menu(self):
        ...
        tk.Button(self.menu_frame, text=f"Table1 - {self.name_table1}", command=lambda: self.show_table(self.name_table1, self.columns1, "1150x450"), height=4, width=35).grid(row=0, column=0)
        ...
        tk.Button(self.menu_frame, text=f"Table3 - {self.name_table3}", command=lambda: self.show_table(self.name_table3, self.columns3, "1150x450"), height=4, width=35).grid(row=2, column=0)
        tk.Button(self.menu_frame, text=f"Table4 - {self.name_table4}", command=lambda: self.show_table(self.name_table4, self.columns4, "950x450"), height=4, width=35).grid(row=3, column=0)
        tk.Button(self.menu_frame, text=f"Table5 - {self.name_table5}", command=lambda: self.show_table(self.name_table5, self.columns5, "1350x475"), height=4, width=35).grid(row=4, column=0)
...
if __name__ == "__main__":
    print("example: 'host': 'localhost'; 'user': 'root'; 'password': '1234567890-'; 'database': 'kursov'")
    host = input("host: ")
    user = input("user: ")
    password = input("password: ")
    database = input("name data base: ")
    connection_params = {"host": host, "user": user, "password": password, "database": database}
    try:
        root = tk.Tk()
